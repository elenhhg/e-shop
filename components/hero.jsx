...
function HeroContent() {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-50 flex items-center justify-center text-foreground">
      <div className="text-center">
        <motion.h1
          ...
          className="text-5xl md:text-7xl font-serif tracking-tight text-[#2f2b2b]"
        >
          HAVEN
        </motion.h1>
        ...
        >
          <Link
            href="/shop"
            className="inline-block px-10 py-4 text-xs font-medium tracking-[0.25em] uppercase border border-foreground/60 backdrop-blur-sm bg-background/20 hover:bg-foreground hover:text-background transition-all duration-500 ease-out"
          >
            Shop Now
          </Link>
        </motion.div>
...
function ParallaxImages() {
  return (
    <div className="mx-auto max-w-5xl px-4 pt-[200px] space-y-40">
      {/* Πρώτη από τις κάτω εικόνες με κείμενο */}
      ...
      <ParallaxImg src="/images/image4.jpg" alt="" start={200} end={-250} className="mx-auto w-2/3" />
...
// Component για εικόνα + κείμενο με διαφορετικό parallax για το text
function ParallaxImgWithText({ className = "w-1/3", alt, src, start, end, text, reverse = false }) {
  ...
  // Παράλληλο but slightly different movement για το text
  ...
  return (
    <motion.div
      ...
      className={`flex flex-col ${reverse ? "lg:flex-row-reverse" : "lg:flex-row"} items-center gap-40 w-full`}
    >
      <img src={src} alt={alt} className={className} />

      <motion.div
        style={{ transform: textTransform }}
        className="max-w-md text-center lg:text-left text-[#2f2b2b]"
      >
        <p className="text-sm tracking-[0.2em] font-light">{text}</p>
      </motion.div>
    </motion.div>
  )
}

// Component για απλή εικόνα χωρίς κείμενο
